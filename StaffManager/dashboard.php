<?php 
    ...
    include 'connect.php'; 
    $staff_manager_id = $_SESSION['staff_manager_id'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Dashboard | STAFF MANAGER | HMC</title>
    ...
    <section class="main-content">
        <div class="profile-container">
            ...
            <div class="profile-info">
                ...
                $staffManagerStmt = $conn->prepare("SELECT * FROM StaffManager WHERE staff_manager_id = '$staff_manager_id' ");
                $staffManagerStmt->execute();
                $staffManager = $staffManagerStmt->fetch(PDO::FETCH_ASSOC);
                ?>
                <h2 id="managerName"><?php echo htmlspecialchars($staffManager['staff_manager_name']); ?></h2>
                <p><b>Staff Manager ID:</b> <span><?php echo htmlspecialchars($staffManager['staff_manager_id']); ?></span></p>
                <p><b>Contact:</b> <span><?php echo htmlspecialchars($staffManager['phone']); ?></span></p>
                <p><b>Email:</b> <span><?php echo htmlspecialchars($staffManager['email']); ?></span></p>
                <button type="button" class="edit-button" onclick="enableEdit()">Edit</button>
            </div>

            <form action="update_staffmanager.php" method="POST" enctype="multipart/form-data" class="form-group">
                <label for="name">Name:</label>
                <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($staffManager['staff_manager_name']); ?>" required>
                <label for="phone">Phone:</label>
                <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($staffManager['phone']); ?>" required>
                <label for="email">Email:</label>
                <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($staffManager['email']); ?>" required>

<?php 
    ...
    include 'connect.php'; 
    $chairman_id = $_SESSION['chairman_id'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Dashboard | CHAIRMAN | HMC</title>
    ...
    <section class="main-content">
        <div class="profile-container">
            ...
            <div class="profile-info">
                ...
                $chairmanStmt = $conn->prepare("SELECT * FROM Chairman WHERE chairman_id = '$chairman_id' ");
                $chairmanStmt->execute();
                $chairman = $chairmanStmt->fetch(PDO::FETCH_ASSOC);
                ?>
                <h2 id="managerName"><?php echo htmlspecialchars($chairman['chairman_name']); ?></h2>
                <p><b>Chairman-ID:</b> <span><?php echo htmlspecialchars($chairman['chairman_id']); ?></span></p>
                <p><b>Contact:</b> <span><?php echo htmlspecialchars($chairman['phone']); ?></span></p>
                <p><b>Email:</b> <span><?php echo htmlspecialchars($chairman['email']); ?></span></p>
                <button type="button" class="edit-button" onclick="enableEdit()">Edit</button>
            </div>

            <form action="update_chairman.php" method="POST" enctype="multipart/form-data" class="form-group">
                <label for="name">Name:</label>
                <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($chairman['chairman_name']); ?>" required>
                <input type="hidden" id="chairman_id" name="chairman_id" value="<?php echo htmlspecialchars($chairman['chairman_id']); ?>" required>
                <label for="phone">Phone:</label>
                <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($chairman['phone']); ?>" required>
                <label for="email">Email:</label>
                <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($chairman['email']); ?>" required>
